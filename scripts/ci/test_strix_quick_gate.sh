...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "PR_HEAD_REF" "strix workflow has PR head branch metadata for missing refs/pull/head fallback"
	assert_file_contains "$workflow_file" "PR_HEAD_REPOSITORY" "strix workflow has PR head repository metadata for fork-aware fallback fetches"
	assert_file_contains "$workflow_file" "[[ \"\$PR_NUMBER\" =~ ^[0-9]+\$ ]]" "strix workflow validates PR number before ref construction"
	assert_file_contains "$workflow_file" "[[ \"\$PR_HEAD_SHA\" =~ ^[0-9a-f]{40}\$ ]]" "strix workflow validates PR head SHA before object checks"
	assert_file_contains "$workflow_file" "check-ref-format \"refs/heads/\$PR_HEAD_REF\"" "strix workflow validates PR head refs before fallback refspec use"
	assert_file_contains "$workflow_file" "refs/heads/\${PR_HEAD_REF}" "strix workflow can fetch the PR head branch when refs/pull/head is unavailable"
	assert_file_contains "$workflow_file" "Fetched PR head SHA does not match event head SHA" "strix workflow explains PR head SHA mismatches"
...
set -euo pipefail
printf 'called\n' >> "${FAKE_STRIX_CALL_LOG:?}"
echo "Error: Strix should not run after a PR-deleted backend context file" >&2
exit 76
EOF
	chmod +x "$fake_strix"
	printf '%s' 'gemini/test-model' >"$strix_llm_file"
	...
	(
		...
		printf '%s\n' 'HEAD_EMAILS_CONTENT_SHOULD_BE_SCANNED' >"$changed_file"
		git add .
		git commit -qm 'head commit deletes backend context'
	)
	local head_sha
	head_sha="$(git -C "$repo_root_dir" rev-parse HEAD)"
	git -C "$repo_root_dir" checkout -q "$base_sha"
	...
	assert_equals "2" "$rc" "case=pull-request-target-deleted-changed-backend-context-fails-closed exit code"
	assert_file_contains "$output_log" "pull request context file could not be read from PR head; failing closed" "case=pull-request-target-deleted-changed-backend-context-fails-closed output"
	local call_count="0"
	if [ -f "$call_log" ]; then
		call_count="$(wc -l <"$call_log" | tr -d ' ')"
	fi
	assert_equals "0" "$call_count" "case=pull-request-target-deleted-changed-backend-context-fails-closed must not invoke Strix"
...
run_pull_request_changed_context_missing_head_commit_uses_checkout_case() {
	local tmp_dir
	tmp_dir="$(mktemp -d)"
	local bin_dir="$tmp_dir/bin"
	local repo_root_dir="$tmp_dir/repo"
...
if ! grep -Fq -- "${FAKE_STRIX_EXPECTED_CONTEXT_CONTENT:?}" "$context_file"; then
	echo "Error: checkout backend context was not scanned" >&2
	cat -- "$context_file" >&2
	exit 77
fi
echo "scan ok with checkout backend context fallback"
EOF
	chmod +x "$fake_strix"
	printf '%s' 'gemini/test-model' >"$strix_llm_file"
	printf '%s' 'dummy' >"$llm_api_key_file"
	printf '%s\n' 'email api content' >"$repo_root_dir/$changed_file"
	printf '%s\n' 'CHECKOUT_AUTH_CONTEXT_SHOULD_BE_SCANNED' >"$repo_root_dir/$context_file"

	set +e
	(
		cd "$repo_root_dir"
		env -u GITHUB_EVENT_PATH -u PR_BASE_SHA \
			PATH="$bin_dir:$PATH" \
			GITHUB_EVENT_NAME="pull_request" \
			...
			FAKE_STRIX_CALL_LOG="$call_log" \
			FAKE_STRIX_EXPECTED_CONTEXT_FILE="$context_file" \
			FAKE_STRIX_EXPECTED_CONTEXT_CONTENT="CHECKOUT_AUTH_CONTEXT_SHOULD_BE_SCANNED" \
			STRIX_PR_SCOPE_MAX_FILES_PER_BATCH="1" \
			STRIX_DISABLE_PR_SCOPING="0" \
			STRIX_LLM_FILE="$strix_llm_file" \
			LLM_API_KEY_FILE="$llm_api_key_file" \
	...
	assert_equals "0" "$rc" "case=pull-request-changed-context-missing-head-commit-uses-checkout exit code"
	if [ "$rc" -ne 0 ]; then
		cat -- "$output_log" >&2
	fi
	assert_file_contains "$output_log" "scan ok with checkout backend context fallback" "case=pull-request-changed-context-missing-head-commit-uses-checkout output"
	local call_count="0"
	if [ -f "$call_log" ]; then
		call_count="$(wc -l <"$call_log" | tr -d ' ')"
	fi
	assert_equals "2" "$call_count" "case=pull-request-changed-context-missing-head-commit-uses-checkout invokes each configured batch"
