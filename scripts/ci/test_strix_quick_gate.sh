...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/collect_failed_check_evidence.sh" 'select((.event // "") == "pull_request_target")' "failed-check evidence only appends PR Strix workflow runs"
	...
	assert_file_contains "$workflow_file" "extract_strix_failed_check_block()" "opencode fallback review scopes provider and cancellation diagnosis to Strix failed-check evidence"
