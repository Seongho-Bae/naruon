...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "statuses: write" "opencode review workflow can publish the required GitHub Actions status context"
	assert_file_contains "$workflow_file" "Publish required OpenCode review status" "opencode review workflow publishes the required status after the review gate"
	assert_file_contains "$workflow_file" 'statuses/${HEAD_SHA}' "opencode review workflow writes the required opencode-review commit status for the current head"
