...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "Use every configured MCP when it is relevant: CodeGraph for structural source evidence, DeepWiki" "opencode review prompt directs the agent to use all configured MCP sources"
	...
	assert_file_contains "$workflow_file" "timeout 1800 opencode run" "opencode review primary model has a bounded extended timeout for larger workflow diffs"
	...
	assert_file_contains "$workflow_file" '$(sed -n '"'"'1,80p'"'"' "$OPENCODE_EVIDENCE_FILE")' "opencode review prompt keeps bounded PR metadata compact for GitHub Models"
	assert_file_contains "$workflow_file" "Return only the sentinel and control block." "opencode review prompt forbids reasoning text before the control sentinel"
	...
	assert_file_contains "$workflow_file" "MODEL: openai-direct/gpt-5.4" "opencode review has an explicit direct OpenAI GPT-5.4 fallback after GitHub Models providers fail"
	assert_file_contains "$workflow_file" "STRIX_OPENAI_API_KEY: \${{ secrets.STRIX_OPENAI_API_KEY }}" "opencode direct OpenAI fallback uses provider-scoped direct OpenAI credentials"
	assert_file_contains "$workflow_file" '"openai-direct"' "opencode review declares a direct OpenAI provider only for explicit GPT-5.4 fallback evidence"
	...
	assert_file_contains "$workflow_file" 'request_changes_for_gate_failure "OpenCode action outcomes were primary=' "opencode review workflow records missing model evidence before failing the gate"
	assert_file_contains "$workflow_file" "Approval gate result was" "opencode review workflow records invalid approval-gate output before failing the gate"
	assert_file_contains "$workflow_file" "exit 1" "opencode review workflow treats missing or invalid review evidence as a hard check failure"
	...
	assert_file_not_contains "$workflow_file" "MODEL: openai-direct/gpt-5.3" "opencode review direct OpenAI fallback must stay at GPT-5.4 or newer"
