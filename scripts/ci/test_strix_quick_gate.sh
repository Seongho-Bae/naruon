...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'ref: ${{ github.event.pull_request.base.sha || github.event.inputs.pr_base_sha || github.sha }}' "opencode review checks out the trusted base workflow instead of the PR head"
