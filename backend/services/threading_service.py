...
async def _find_existing_thread_id(
    session: AsyncSession, message_id: str, user_id: str | None = None
) -> str | None:
    ...
    if user_id is not None:
        stmt = stmt.where(Email.user_id == user_id)
    result = await session.execute(stmt)
    return result.scalar_one_or_none()


async def assign_thread_id(
    session: AsyncSession, email_data: EmailData, user_id: str | None = None
) -> str:
    """
    ...
    for candidate in existing_candidates:
        thread_id = await _find_existing_thread_id(session, candidate, user_id=user_id)
        if thread_id:
