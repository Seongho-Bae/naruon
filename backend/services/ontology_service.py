...
class OntologyService:
    ...
    def analyze_sender_relationship(
        ...
        logger.info(
            f"Analyzed relationship: {sender_email} -> {relationship_type} (conf: {confidence})"
        )
        return {"type": relationship_type, "confidence": confidence}
    ...
    async def process_knowledge_node(
        self, session, email_data: dict, user_id: str, organization_id: str | None
    ) -> bool:
        # Pseudo implementation for knowledge extraction trigger
        logger.info(
            f"Triggering knowledge extraction for user {user_id} based on self-to-self email."
        )
