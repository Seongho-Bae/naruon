from .adapter_utils import create_adapter_error
import ipaddress
import socket
...
class LocalDavAdapters:
    ...
    async def _put(
        ...
        *,
        ...
        if source is None:
            return create_adapter_error("source_not_configured")
        if not source.writeback_enabled:
            return create_adapter_error("source_writeback_disabled")

        if_match = self._payload_text(payload, "if_match")
        if if_match is None:
            return create_adapter_error("missing_if_match")

        target_path = self._safe_target_path(payload.get("target_path"))
        if target_path is None:
            return create_adapter_error("invalid_target_path")

        content = self._payload_content(payload.get("content"))
        if content is None:
            return create_adapter_error("invalid_payload")

        try:
            target_url = self._target_url(source.base_url, target_path)
        except ValueError as exc:
            return create_adapter_error(str(exc))
        ...
        try:
            ...
            return create_adapter_error("provider_request_failed")
    ...
    def _result_from_response(self, response) -> dict[str, Any]:
        ...
        result = create_adapter_error("provider_write_failed")
