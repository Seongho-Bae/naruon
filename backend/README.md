...
Set `DATABASE_URL` in `.env`; startup fails closed when the database DSN is
missing.
...
`ALTER TABLE ... ADD COLUMN IF NOT EXISTS` statements for the threading columns
(`thread_id`, `in_reply_to`, `references`, `reply_to`) plus the thread index.
Existing email rows that predate threading should then be backfilled by
...
Before claiming multi-user safety, add an owner/mailbox column to `emails`,
backfill it, and scope every email/search query by that key.

## Warning classification

Current backend test warnings are known dependency/toolchain warnings:

- `starlette.formparsers` imports `multipart`, which emits a pending
  deprecation warning.
