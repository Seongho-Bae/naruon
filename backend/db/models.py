...
from core.config import settings, validate_encryption_key_value
...
def get_fernet() -> Fernet:
    if settings.ENCRYPTION_KEY is not None:
        ...
        try:
            validate_encryption_key_value(key_value)
            return Fernet(key)
        except ValueError as exc:
            raise RuntimeError(
                "ENCRYPTION_KEY must be a valid Fernet key with high entropy generated by "
                "Fernet.generate_key()."
            ) from exc
    raise RuntimeError(
