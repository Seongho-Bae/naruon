...
def test_determine_writeback_target():
    ...
    target_1 = caldav_service.determine_writeback_target(
        task_context_1, connected_accounts
    )
    assert target_1 == "caldav_src_company"

    # Should fallback to primary
    task_context_2 = {"source_email": "friend@other.com"}
    target_2 = caldav_service.determine_writeback_target(
        task_context_2, connected_accounts
    )
    ...
    target_3 = caldav_service.determine_writeback_target(
        task_context_3, connected_accounts
    )
