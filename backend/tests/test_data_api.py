...
def _with_signed_auth(mock_db, token: str):
    ...
    def override_require_workspace(auth_context):
        pass
    app.dependency_overrides[security_api._require_authoritative_workspace_scope] = override_require_workspace

    from api.auth import _verify_signed_session_payload, _auth_context_from_session_payload
    def override_get_auth_context():
        payload, _ = _verify_signed_session_payload(f"Bearer {token}")
        return _auth_context_from_session_payload(payload, "override")
