...
def test_validate_imap_host(monkeypatch):
    ...
    assert (
        validate_imap_host("imap.example.com", resolve_host=False) == "imap.example.com"
    )
...
def test_validate_pop3_host(monkeypatch):
    ...
    assert (
        validate_pop3_host("pop3.example.com", resolve_host=False) == "pop3.example.com"
    )
...
def test_validate_imap_host_resolve(monkeypatch):
    ...
    assert (
        validate_imap_host("imap.example.com", resolve_host=True) == "imap.example.com"
    )
...
def test_validate_pop3_host_resolve(monkeypatch):
    ...
    monkeypatch.setattr(socket, "getaddrinfo", mock_getaddrinfo)
    assert (
        validate_pop3_host("pop3.example.com", resolve_host=True) == "pop3.example.com"
    )
...
def test_resolve_all_public_mail_addresses_empty_result(monkeypatch):
    ...
    monkeypatch.setattr(socket, "getaddrinfo", mock_getaddrinfo)
    with pytest.raises(ValueError, match=IMAP_HOST_NOT_ALLOWED):
        email_client._resolve_all_public_mail_addresses(
            "imap.example.com", 993, IMAP_HOST_NOT_ALLOWED
        )
...
def test_normalize_mail_host_invalid_chars():
    with pytest.raises(ValueError, match=IMAP_HOST_NOT_ALLOWED):
        email_client._normalize_mail_host(
            "imap.example.com/path", IMAP_HOST_NOT_ALLOWED
        )
...
def test_normalize_mail_host_localhost():
    ...
    with pytest.raises(ValueError, match=IMAP_HOST_NOT_ALLOWED):
        email_client._normalize_mail_host(
            "localhost.localdomain", IMAP_HOST_NOT_ALLOWED
        )
...
def test_validate_smtp_destination_no_resolve(monkeypatch):
    ...
    dest = email_client.validate_smtp_destination(
        "smtp.example.com", 587, resolve_host=False
    )
...
def test_resolve_smtp_connect_address_empty(monkeypatch):
    monkeypatch.setattr(
        email_client, "_resolve_all_public_smtp_addresses", lambda host, port: []
    )
    with pytest.raises(ValueError, match=email_client.SMTP_HOST_NOT_ALLOWED):
...
async def test_connect_validated_smtp_socket_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        email_client.asyncio.get_running_loop(), "sock_connect", mock_sock_connect
    )
...
async def test_pinned_implicit_tls_smtp_not_socket(monkeypatch):
    ...
    monkeypatch.setattr(
        email_client.aiosmtplib.SMTP, "_create_connection", mock_super_create_connection
    )
...
async def test_pinned_implicit_tls_smtp_connect_timeout(monkeypatch, smtp_socket):
    ...
    monkeypatch.setattr(
        email_client.asyncio.get_running_loop(),
        "create_connection",
        lambda *args, **kwargs: mock_wait_for(None, None),
    )

    with pytest.raises(
        email_client.SMTPConnectTimeoutError,
        match="Timed out connecting to smtp.example.com",
    ):
...
async def test_pinned_implicit_tls_smtp_connect_oserror(monkeypatch, smtp_socket):
    ...
    with pytest.raises(
        email_client.SMTPConnectError,
        match="Error connecting to smtp.example.com: network down",
    ):
        await client._create_connection(timeout=10.0)


@pytest.mark.asyncio
async def test_pinned_implicit_tls_smtp_read_response_disconnected(
    monkeypatch, smtp_socket
):
    sock = smtp_socket
    client = email_client._PinnedImplicitTlsSMTP(
        tls_server_hostname="smtp.example.com",
    ...
    with pytest.raises(
        email_client.SMTPConnectError,
        match="Error connecting to smtp.example.com: disconnected",
    ):
        await client._create_connection(timeout=10.0)
...
async def test_pinned_implicit_tls_smtp_read_response_timeout(monkeypatch, smtp_socket):
    ...
    with pytest.raises(
        email_client.SMTPConnectTimeoutError,
        match="Timed out waiting for server ready message",
    ):
        await client._create_connection(timeout=10.0)


@pytest.mark.asyncio
async def test_pinned_implicit_tls_smtp_read_response_not_ready(
    monkeypatch, smtp_socket
):
...
def test_build_smtp_client_non_465(smtp_socket):
    ...
    client = email_client._build_smtp_client(
        smtp_socket=sock, smtp_server="smtp.example.com", smtp_port=587
    )
