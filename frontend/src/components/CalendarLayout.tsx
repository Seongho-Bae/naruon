...
export function CalendarLayout() {
  ...
  const selectedDetailTitle = toSafeReactText(selectedDetailEvent?.title, '표시 중인 일정 없음');
  const selectedDetailSource = toSafeReactText(selectedDetailEvent?.source, '표시 중인 원본 없음');
  const selectedDetailDescription = toSafeReactText(selectedDetailEvent?.description, '표시할 일정 설명이 없습니다.');
  const selectedDetailTime = toSafeReactText(selectedDetailEvent?.time, '--:--');
  const selectedDetailDuration = toSafeReactText(selectedDetailEvent?.duration, '일정 없음');
  const selectedDetailLocation = toSafeReactText(selectedDetailEvent?.location, '장소 없음');
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-4 pb-[calc(7rem+env(safe-area-inset-bottom))] md:p-6 lg:pb-6">
          ...
          {viewMode === '일정 상세' && (
            <section aria-label="일정 상세" className="rounded-2xl border border-border bg-card p-5 shadow-sm">
              <h3 className="text-lg font-bold">{selectedDetailEvent ? `${selectedDetailTitle} 상세` : '일정 상세'}</h3>
              <dl className="mt-4 grid gap-4 md:grid-cols-2">
                <div className="rounded-xl border border-border bg-background p-4">
                  <dt className="text-xs font-black text-muted-foreground">원본 계정</dt>
                  <dd className="mt-2 text-sm font-bold">{selectedDetailEvent ? `${selectedDetailSource} · 충돌 토큰 확인` : '표시 중인 원본 없음'}</dd>
                </div>
                <div className="rounded-xl border border-border bg-background p-4">
      ...
      <aside className="w-[340px] shrink-0 flex-col overflow-y-auto border-l border-border bg-card p-5 hidden xl:flex">
        ...
        <div className="mt-6">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold">{selectedDetailEvent ? `${selectedDetailTitle} (Naruon 2.0)` : '표시 중인 일정 없음'}</h2>
          </div>
          <p className="mt-2 text-sm text-muted-foreground">{selectedDetailEvent ? selectedDetailDescription : '왼쪽 캘린더 목록에서 하나 이상의 캘린더를 표시하세요.'}</p>
        </div>
        ...
        <div className="mt-6 space-y-5">
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{selectedDetailDuration}</p>
            </div>
          </div>
          <div className="flex gap-3 items-center">
            <Video className="size-5 text-muted-foreground shrink-0" />
            <p className="text-sm font-semibold">{selectedDetailLocation}</p>
            <button type="button" aria-label={`${selectedDetailLocation} 위치 보기`} className="text-xs text-primary font-semibold ml-auto hover:underline rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40">위치 보기</button>
          </div>
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{selectedDetailDescription}</p>
            </div>
          </div>
          <div className="flex gap-3 items-start">
