...
function getApiErrorStatus(error: unknown) {
  ...
  if (typeof shapedError?.response?.status === "number")
    return shapedError.response.status;
  return null;
}

export function CalendarLayout() {
  const [viewMode, setViewMode] = useState<
    "월간 캘린더" | "주간 캘린더" | "일정 상세" | "회의 조율" | "일정 후보"
  >("월간 캘린더");
  const [writebackStatus, setWritebackStatus] =
    useState<WritebackStatus>("idle");
  const [writebackResult, setWritebackResult] =
    useState<CalendarWritebackIntentResponse | null>(null);
  const [writebackSources, setWritebackSources] = useState<
    CalendarWritebackSource[]
  >([]);
  const [sourceLoadStatus, setSourceLoadStatus] = useState<
    "loading" | "ready" | "error"
  >("loading");
  ...
  useEffect(() => {
    let isMounted = true;
    void apiClient
      .get<CalendarWritebackSource[]>("/api/calendar/writeback-sources")
      .then((sources) => {
        if (!isMounted) return;
        setWritebackSources(sources);
        setSelectedSourceId(
          sources.find(isCustomerOwnedWritableSource)?.source_id ?? null,
        );
  ...
  const requestWritebackIntent = useCallback(
    async (action: "create" | "update") => {
      ...
      setWritebackResult(null);
      try {
        const result = await apiClient.post<CalendarWritebackIntentResponse>(
          "/api/calendar/writeback-intent",
          {
            ...
            ...(selectedWritebackSource
              ? { target_source_id: selectedWritebackSource.source_id }
              : {}),
          },
        );
  ...
  const isWritebackActionDisabled =
    isWritebackLoading || !isSourceRegistryReady;
  ...
  return (
    <div className="flex h-full min-h-0 bg-background text-foreground">
      ...
      <aside className="w-64 shrink-0 flex-col overflow-y-auto border-r border-border bg-card p-4 hidden lg:flex">
        <button
          type="button"
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2.5 text-sm font-bold text-primary-foreground shadow-sm hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
        >
        ...
        <div className="mt-8">
          ...
          <button
            type="button"
            className="mt-4 flex items-center gap-2 text-sm font-semibold text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
          >
      ...
      <main className="flex min-w-0 flex-1 flex-col bg-background">
        <header className="flex h-auto min-h-16 shrink-0 flex-col items-start gap-3 border-b border-border bg-card px-4 py-3 lg:px-6">
          <div className="flex min-w-0 flex-wrap items-center gap-3 lg:gap-4">
            <button
              type="button"
              className="rounded-md border border-border bg-background px-3 py-1.5 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
            >
              오늘
            </button>
            <div className="flex items-center gap-1">
              <button
                type="button"
                aria-label="이전 달"
                className="grid size-8 place-items-center rounded-md hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
              >
            ...
            <h2 className="text-sm font-bold text-muted-foreground lg:ml-2">
              2026년 5월
            </h2>
          </div>
          <div className="flex w-full min-w-0 items-center gap-3">
            ...
            <button
              type="button"
              aria-label="설정"
              className="grid size-9 shrink-0 place-items-center rounded-md border border-border bg-background hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
            >
        ...
        <div className="flex-1 space-y-5 overflow-y-auto p-4 pb-[calc(7rem+env(safe-area-inset-bottom))] md:p-6 lg:pb-6">
          ...
          <section
            aria-label="CalDAV writeback intent 점검"
            className="rounded-2xl border border-border bg-card p-4 shadow-sm md:p-5"
          >
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              <div className="min-w-0">
                <p className="text-xs font-black uppercase text-primary">
                  Customer-owned calendar intent
                </p>
                <h2 className="mt-1 text-lg font-black text-foreground">
                  CalDAV/CardDAV/WebDAV writeback intent
                </h2>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
                  Naruon은 캘린더 서버가 아니라 고객 원본 계정에 반영할 의도를
                  기록합니다. 실제 provider write는 source capability,
                  provenance, ETag/If-Match, 감사 이벤트를 통과해야 합니다.
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-black ${sourceWritable ? "bg-green-100 text-green-700" : "bg-secondary text-muted-foreground"}`}
                      >
                        {sourceSelected
                          ? "selected"
                          : sourceWritable
                    ...
                    <p className="mt-2 break-all font-mono text-[11px] font-semibold text-muted-foreground">
                      etag={source.etag ?? "missing"} · writeback=
                      {sourceWritable ? "eligible" : "blocked"}
                    </p>
                  </button>
                );
              })}
              ...
              {sourceLoadStatus === "error" && (
                <p className="rounded-xl border border-border bg-background/70 p-3 text-sm font-bold text-amber-700">
                  signed session으로 CalDAV source registry를 확인할 수
                  없습니다.
                </p>
              )}
            </div>
            ...
            <div
              role="status"
              aria-live="polite"
              className="mt-4 rounded-xl border border-border bg-background/70 p-4 text-sm"
            >
              {writebackStatus === "idle" && (
                <p className="text-muted-foreground">
                  아직 provider write는 실행하지 않았습니다. intent 점검으로
                  원본 source와 충돌 조건만 확인합니다.
                </p>
              )}
              {writebackStatus === "loading" && (
                <p className="font-bold text-primary">
                  writeback intent 요청 중입니다.
                </p>
              )}
              {writebackStatus === "no_source" && (
                <p className="font-bold text-amber-700">
                  원본 CalDAV/CardDAV/WebDAV 계정이 없어 writeback intent를 만들
                  수 없습니다.
                </p>
              )}
              {writebackStatus === "conflict" && (
                <p className="font-bold text-red-700">
                  ETag/If-Match 충돌이 감지되어 원본 일정을 덮어쓰지 않았습니다.
                </p>
              )}
              {writebackStatus === "auth" && (
                <p className="font-bold text-red-700">
                  signed session이 필요합니다. 공개 헤더로는 writeback intent를
                  만들 수 없습니다.
                </p>
              )}
              {writebackStatus === "error" && (
                <p className="font-bold text-red-700">
                  writeback intent 점검에 실패했습니다.
                </p>
              )}
                ...
                <dl className="grid gap-3 text-xs sm:grid-cols-2 2xl:grid-cols-3">
                  <div>
                    ...
                    <dd className="mt-1 font-mono text-sm text-foreground">
                      {writebackResult.writeback_mode}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-black text-muted-foreground">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 font-mono text-sm text-foreground">
                      {writebackResult.target_source_id}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-black text-muted-foreground">
                    ...
                    <dd className="mt-1 font-mono text-sm text-foreground">
                      {writebackResult.if_match ?? "not_required"}
                    </dd>
                  </div>
                  <div>
                    <dt className="font-black text-muted-foreground">
              ...
              <div className="grid grid-cols-7 grid-rows-5 flex-1 divide-x divide-y divide-border">
                ...
                {Array.from({ length: 35 }).map((_, i) => (
                  <div key={i} className="min-h-[84px] p-2 sm:min-h-[100px]">
                    <span
                      className={`text-sm font-semibold ${i % 7 === 0 ? "text-red-500" : i % 7 === 6 ? "text-blue-500" : "text-muted-foreground"}`}
                    >
                      {i < 31 ? i + 1 : ""}
                    </span>
                    {i === 15 && (
                      <div className="mt-1 rounded bg-green-100 px-1.5 py-1 text-[10px] font-semibold leading-tight text-green-700 sm:px-2 sm:text-xs">
                        10:00
                        <span className="hidden sm:inline"> 제품 리뷰</span>
                      </div>
                    )}
                    {i === 22 && (
                      <div className="mt-1 rounded bg-orange-100 px-1.5 py-1 text-[10px] font-semibold leading-tight text-orange-700 sm:px-2 sm:text-xs">
                        09:30
                        <span className="hidden sm:inline"> 출시 회의</span>
                      </div>
                    )}
                  </div>
                ))}
            ...
            >
              ...
              <dl className="mt-4 grid gap-4 md:grid-cols-2">
                <div className="rounded-xl border border-border bg-background p-4">
                  <dt className="text-xs font-black text-muted-foreground">
                    원본 계정
                  </dt>
                  <dd className="mt-2 text-sm font-bold">
                    Customer CalDAV · caldav-primary
                  </dd>
                </div>
                <div className="rounded-xl border border-border bg-background p-4">
                  <dt className="text-xs font-black text-muted-foreground">
                  ...
                  <dd className="mt-2 text-sm font-bold">
                    ETag / If-Match 필요 시 server-authoritative 검증
                  </dd>
                </div>
              </dl>
            </section>
            ...
            <div className="flex h-full flex-col gap-4">
              <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  참석자들의 캘린더(CalDAV)를 종합 분석하여 최적의 시간을
                  제안합니다.
                </p>
                <div className="grid gap-3 max-w-lg">
                  <button
                    type="button"
                    className="flex items-center justify-between rounded-xl border border-primary/20 bg-primary/5 p-4 hover:bg-primary/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
                  >
                  ...
                  <button
                    type="button"
                    className="flex items-center justify-between rounded-xl border border-border bg-card p-4 hover:bg-secondary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
                  >
                    <div className="flex items-center gap-3">
                      <span className="grid size-8 place-items-center rounded-lg bg-secondary text-muted-foreground font-bold">
                        2안
                      </span>
                      <div className="text-left">
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-3">
                {[
                  ...
                  >
                    ...
                    <p className="mt-3 rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary">
                      {mode}
                    </p>
                  </article>
                ))}
      ...
      <aside className="w-[340px] shrink-0 flex-col overflow-y-auto border-l border-border bg-card p-5 hidden xl:flex">
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="flex gap-3 items-center">
            ...
            <button
              type="button"
              className="text-xs text-primary font-semibold ml-auto hover:underline rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
            >
              위치 보기
            </button>
          </div>
            ...
            <div>
              ...
              <div className="flex -space-x-2">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div
                    key={i}
                    className="size-8 rounded-full border-2 border-card bg-slate-200"
                  ></div>
                ))}
                <div className="flex size-8 items-center justify-center rounded-full border-2 border-card bg-secondary text-xs font-bold">
                  +2
                </div>
              </div>
            </div>
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Naruon 2.0 출시 전 최종 점검 및 공유, 각 파트별 일정 및 역할
                확인.
              </p>
            </div>
          </div>
        ...
        <div className="mt-8 flex gap-3">
          <button
            type="button"
            className="flex-1 rounded-lg border border-border bg-background py-2 text-sm font-bold shadow-sm hover:bg-secondary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
          >
            삭제
          </button>
          <button
          ...
          <button
            type="button"
            className="flex-1 rounded-lg bg-primary py-2 text-sm font-bold text-primary-foreground shadow-sm hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
          >
            수정
          </button>
        </div>
