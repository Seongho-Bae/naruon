    ...
    <div className="flex h-full min-h-0 bg-background text-foreground">
      ...
      <aside className="w-[340px] shrink-0 flex-col overflow-y-auto border-l border-border bg-card p-5 hidden xl:flex">
        ...
        <div className="mt-6">
          ...
          <p className="mt-2 text-sm text-muted-foreground">{selectedDetailEvent ? toSafeReactText(selectedDetailEvent.description) : '왼쪽 캘린더 목록에서 하나 이상의 캘린더를 표시하세요.'}</p>
        </div>
        ...
        <div className="mt-6 space-y-5">
          ...
          <div className="flex gap-3 items-center">
            ...
            <p className="text-sm font-semibold">{selectedDetailEvent ? toSafeReactText(selectedDetailEvent.location) : '장소 없음'}</p>
            <button type="button" aria-label={`${selectedDetailEvent ? toSafeReactText(selectedDetailEvent.location) : '장소'} 위치 보기`} className="text-xs text-primary font-semibold ml-auto hover:underline rounded-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40">위치 보기</button>
          </div>
          ...
          <div className="flex gap-3 items-start">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{selectedDetailEvent ? toSafeReactText(selectedDetailEvent.description) : '표시할 일정 설명이 없습니다.'}</p>
            </div>
          </div>
          <div className="flex gap-3 items-start">
