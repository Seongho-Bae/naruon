      ...
      <main className="flex-1 overflow-x-auto overflow-y-hidden p-6 bg-secondary/20">
        <div className="flex h-full gap-6">
          {KANBAN_COLUMNS.map((col) => (
            <div key={col.id} className="flex h-full w-80 flex-col rounded-xl bg-card border border-border shadow-sm shrink-0">
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3">
                {MOCK_TASKS[col.id as keyof typeof MOCK_TASKS].map((task) => (
                  <div key={task.id} className="cursor-pointer rounded-lg border border-border bg-background p-3 shadow-sm hover:border-primary/50 hover:shadow-md transition-all">
                    ...
                    <div className="mt-3 flex items-center justify-between text-xs font-semibold text-muted-foreground">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1" title="출처">
