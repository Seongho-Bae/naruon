...
export function TasksLayout() {
  ...
  // ⚡ Bolt: Memoize flattened tasks to prevent O(n) recalculations on every render
  const allTasks = useMemo(() => Object.values(tasks).flat(), [tasks]);
  const myTasks = useMemo(() => allTasks.filter(t => t.assignee === '김나루'), [allTasks]);
  const delegatedTasks = useMemo(() => allTasks.filter(t => t.assignee !== '김나루'), [allTasks]);
