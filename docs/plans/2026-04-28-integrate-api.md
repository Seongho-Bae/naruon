...
export function EmailDetail({ emailId }: { emailId: number | null }) {
  ...
  return (
    <div className="p-6 h-full flex flex-col gap-6 overflow-y-auto">
      ...
      {/* prettier-ignore */}
      <td>Creates a calendar event (VTODO) for each execution item securely parsed by the LLM. Rejects unsafe execution text using allowlists before attempting creation.</td>
      <Card>
        <CardHeader>
