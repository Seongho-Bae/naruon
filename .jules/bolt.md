...
## 2026-06-02 - LRU cache on slow email.utils parsing methods
**Learning:** Functions like `email.utils.parseaddr` and `email.utils.getaddresses` are heavily hit when processing many emails in large threads (e.g. `get_emails` API or `reply_tracking_service`). The stdlib string manipulation overhead in these functions scales poorly.
**Action:** When a method parses identical strings heavily across items in an API cycle (e.g. the same sender string for thread items), apply `functools.lru_cache` on a lightweight helper to retrieve the parsed output and achieve near O(1) returns. Use `frozenset` instead of `set` if parsing arrays, since `frozenset` is natively hashable and allows caching.
