...
## 2025-05-30 - O(N^2) optimization in TasksLayout and useMemo in frontend graph
**Learning:** `TasksLayout.tsx` repeatedly executed `Object.values(tasks).flat()` inside mapping/filtering callbacks directly in the React render cycle, resulting in redundant O(N) operations and array allocations on every render.
**Action:** When filtering objects mapped iteratively, identify overlapping inner iterators (like checking for matching inner properties across items) and build them in a lookup dictionary ahead of time. In React, safely memoize constant properties built sequentially by extracting them to a `useMemo` hook at the component level to prevent repeated execution during renders.
