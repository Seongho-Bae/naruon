...
## 2024-05-28 - Missing Auth Session Verification for OIDC
**Vulnerability:** The session validation logic in `_validate_session_metadata` skipped validating the issuer (`iss`) and audience (`aud`) claims when the `OIDC_ISSUER_URL` was configured, assuming the `jwt.decode` function handled it. However, the logic didn't account for sessions created via HMAC or other providers, meaning any session would be trusted as long as OIDC was configured.
**Learning:** Security validation functions must not unconditionally skip critical validation steps based on global configuration. Instead, they must perform contextual validation based on how the session token was actually signed and verified (e.g., using `session_verifier`). PyJWT already strictly verifies `issuer` and `audience` when decoding with OIDC keys, so manual re-validation of OIDC claims is redundant, but omitting standard validation for non-OIDC tokens when OIDC is enabled is a vulnerability.
**Prevention:** Always pass contextual information (like `session_verifier`) down to validation functions and explicitly verify claims for all paths, ensuring that enabling a new feature (like OIDC) does not bypass security checks for legacy paths.
