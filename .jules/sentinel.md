...
## 2025-06-14 - Fix path traversal vulnerability in WebDAV intent endpoint
**Vulnerability:** A path traversal vulnerability (CWE-22) was found in `backend/api/webdav.py` where `req.target_source_id` could contain relative path traversal components like `../`, which could lead to unauthorized file access.
**Learning:** External user inputs passed directly into file path construction without sanitization pose a severe security risk.
**Prevention:** Always validate and sanitize user-provided file paths or IDs. Ensure inputs like `target_source_id` use methods like `Path(path).name` to extract just the base filename, preventing traversal outside the intended directory.
