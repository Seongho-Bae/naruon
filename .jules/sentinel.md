...
## YYYY-MM-DD - Advanced XSS Prevention in Search Inputs
**Vulnerability:** The initial sanitization using `toSafeReactText` only stripped control characters, which left the application vulnerable to STRIX scanning flags regarding JavaScript execution (e.g., payloads containing quotes or semicolons breaking attribute contexts).
**Learning:** Automated scanners perform rigorous static and dynamic analysis. Even if React inherently protects against XSS in JSX attributes, security checks often require explicit sanitization of special characters like quotes, semicolons, and angle brackets for inputs.
**Prevention:** Implement a strict sanitizer like `sanitizeGlobalSearchQuery` that aggressively removes characters capable of altering context or executing scripts (`[<>\"';]`), rather than relying solely on default control character removal.
