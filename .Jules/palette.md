...
## 2024-05-23 - Calendar Writeback Intent UX
**Learning:** Added visual loading spinners (`Loader2`) to async buttons (writeback intent checks) to improve feedback, ensuring `isWritebackLoading` controls the spinner state.
**Action:** Always add visual indicators like `animate-spin` to action buttons triggering async operations to prevent duplicate clicks and improve accessibility.
