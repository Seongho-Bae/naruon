...
## 2024-06-23 - Clean Commits for UI Verification
**Learning:** During frontend UI verification, I heavily utilized Playwright scripts, Next.js server logs, and screenshot captures to debug visual states. However, I inadvertently included these debug artifacts in the initial code review patch, polluting the git history.
**Action:** Always run `git status` after completing verification and explicitly delete or `git ignore` any temporary debug files (screenshots, dummy tests, logs) before staging the final clean patch to ensure a maintainable and professional commit history.
