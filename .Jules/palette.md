...
## 2026-06-27 - Apply aria-busy to Prompt Studio Action Buttons
**Learning:** Adding the `aria-busy` attribute to buttons that trigger asynchronous operations (like prompt generation or saving) greatly improves the experience for users relying on assistive technologies by actively signaling processing state.
**Action:** Always include `aria-busy={isLoading}` on components that initiate asynchronous tasks.
