...
## 2024-05-23 - 캘린더 Writeback Intent UX 피드백 개선
**Learning:** 비동기 작업인 writeback intent 버튼에 시각적 로딩 스피너(`Loader2`)와 텍스트(예: "점검 중")를 추가하여 사용자 피드백을 향상시켰으며 `isWritebackLoading`으로 상태를 제어하였습니다.
**Action:** 비동기 작업을 트리거하는 액션 버튼에는 항상 `animate-spin` 같은 시각적 인디케이터를 추가하여 중복 클릭을 방지하고 접근성을 향상시키십시오.
